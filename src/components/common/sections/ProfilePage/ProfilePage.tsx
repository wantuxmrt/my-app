...
const ProfileSection = () => {
  ...
  useEffect(() => {
    const fetchTickets = async () => {
      try {
        const response = await fetch('/mockData/tickets.json');
  ...
  useEffect(() => {
    if (currentUser) {
      ...
      const userRequests = tickets.filter(t => t.userId === currentUser.id);
      setStats({
      ...
      <div className={styles.profileInfo}>
        <div className={styles.avatarSection}>
          <div className={styles.avatar}>
            {userData.avatar ? (
              <div className={styles.avatarInitials}>{userData.avatar}</div>
            ) : (
          ...
          <div className={styles.formGroup}>
            ...
            <p>
              {userData.role === 'admin' ? 'Администратор' : 
               userData.role === 'support' ? 'Поддержка' : 
